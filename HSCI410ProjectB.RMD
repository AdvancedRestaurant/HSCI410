...
#Tables for report
depdatafull <- data.frame(matrix(ncol = 3, nrow = 60))
...
depdatafull %>% 
  ...
  pack_rows("Depression High/Low", 8, 9) %>%
  pack_rows("Sex", 10, 11) %>%
  pack_rows("Racial background", 12, 13) %>%
  pack_rows("Martial Status", 14, 17) %>%
  pack_rows("Education", 18, 20) %>%
  pack_rows("Work Status", 21, 22) %>%
  pack_rows("Household income", 23, 27) %>%
  pack_rows("Work stress", 28, 30) %>%
  pack_rows("Work group", 31, 35) %>%
  pack_rows("Perceived general health", 36, 40) %>%
  pack_rows("Perceived mental health", 41, 45) %>%
  pack_rows("BMI", 46, 49) %>%
  pack_rows("Drinking Frequency", 50, 51) %>%
  pack_rows("Smoking status", 52, 54) %>%
  pack_rows("Smoking Frequency", 55, 60)
