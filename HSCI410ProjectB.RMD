...
#Forward stepwise regression to determine variables to add 22 variables
...
wrkhours.scale <- (depdata18to64.test$LBFDGHPW/10)

dep.wrkhoursfull.scale<- glm(formula = depdata18to64.test$groupdep ~ wrkhours.scale + depdata18to64.test$wrkstress.skill + 
                         depdata18to64.test$wrkstress.security + depdata18to64.test$wrkstress.authority + depdata18to64.test$wrkstress.social + 
                         factor.incomehousehold + depdata18to64.test$agebracket +depdata18to64.test$SDCDGCGT + factor.smkrtype + factor.bmi + 
                         depdata18to64.test$Life_stress, family = "binomial")
...
workhours.crude.scale <- glm(depdata18to64.test$groupdep ~ wrkhours.scale, family = "binomial")
...
view(dfSummary(depdata.summary1, plain.ascii = FALSE, style = "grid", 
               graph.magnif = 0.75, valid.col = FALSE, tmp.img.dir = "/tmp"))

#Tables for report
depdatafull <- data.frame(matrix(ncol = 3, nrow = 41))

colnames(depdatafull) <- c ("", "","")
rownames(depdatafull) <- c("No depression",
                               "Minimal depression",
                               "Mild depression",
                               "Moderate depression",
...
#Second table
depdatafullb <- data.frame(matrix(ncol = 3, nrow = 11))

colnames(depdatafullb) <- c ("", "","")
rownames(depdatafullb) <- c("Average hours worked per week",
                               "Full Time",
                               "Part Time",
                               "No work stress",
                               "Minimal work stress",
                               ...
                               "NOC 8,9")
#808
depdata18to29b <- depdata.summary1 %>%
  filter(agebracket == "[18-29]")
#1600
depdata30to49b <- depdata.summary1 %>%
  filter(agebracket == "[30-49]")
#1025
depdata50to64b <- depdata.summary1 %>%
  filter(agebracket == "[50-64]")

dep18to29b <- c("41 (2 - 98)",
...
depdatafullb[,1] <- dep18to29b
depdatafullb[,2] <- dep30to49b
depdatafullb[,3] <- dep50to64b


#Final frequency table for report

depdatafull %>% 
  kbl(caption = "<b>Table 1: Socialdemographic and lifestyle data from participants from the 2015-2016 Canadian Community Healthy Survey, by age group</b>",format = "html") %>%
  ...
  pack_rows("Depression severity", 1, 6) %>%
  pack_rows("Sex", 7, 8) %>%
  pack_rows("Racial background", 9, 10) %>%
  pack_rows("Martial Status", 11, 14) %>%
  pack_rows("Education", 15, 17) %>%
  
  pack_rows("Household income", 18, 22) %>%
  pack_rows("Perceived general health", 23,27) %>%
  pack_rows("Perceived mental health", 28, 32) %>%
  pack_rows("BMI", 33, 36) %>%
  pack_rows("Drinking Frequency", 37, 38) %>%
  pack_rows("Smoking status", 39, 41) %>%
  footnote(general = "Number of individuals (n) used in study = 3433",
           general_title = "General:",
           title_format = c("italic","underline")
  )
...
#Table 2
depdatafullb %>% 
  kbl(caption = "<b>Table 2: Work related data from participants from the 2015-2016 Canadian Community Healthy Survey, by age group</b>",format = "html") %>%
  kable_classic(full_width = FALSE,position = "center") %>%
  add_header_above(c(" "=1,"(n = 808)"=1,"(n = 1600)"=1,"(n = 1025)"=1)) %>%
  add_header_above(c(" "=1,"18-29"=1,"30-49"=1,"50-64"=1))%>%
  add_header_above(c(" "=1, "Age group,yr; frequency of respondents (with percentages)"=3)) %>%
  pack_rows("Work status", 2, 3) %>%
  pack_rows("Work Stress", 4, 6) %>%
  pack_rows("NOC skill types", 7, 11) %>%
  footnote(general = "Number of individuals (n) used in study = 3433, National Occupational Classification (NOC)",
           number = c("Business, Finance, and Administration, Health occupation, Education, Law and social,
                      Community and Government Services, Art, Culture, Recreation, Sport, Sales and service",
                      "Natural and applied sciences",
                      "Sales and service occupations",
                      "Trade, transport and equipment operators",
                      "Natural resource and Agriculture, Manufacturing and utilties"),
           general_title = "General:",
           number_title = c("NOC following order of the table 2"),
           title_format = c("italic","underline")
  )
...
dep.resultsor <- data.frame(matrix(ncol = 2, nrow = 33))

rownames(dep.resultsor) <- c("Hours worked per week",
                             "High job skill",
                             "Medium job skill",
                             "Low job skill",
                             "High job security",
...
AdjustedORb <- c("0.817 (0.728 - 0.914)",
                 "1.00",
                 "2.074 (1.358 - 3.278)",
                 "3.924 (2.229 - 6.984)",
                 "1.00",
...
dep.resultsor[,1] <- CrudeORb
dep.resultsor[,2] <- AdjustedORb
colnames(dep.resultsor) <- c ("", "")


#Final OR table for report
dep.resultsor %>% 
  kbl(caption = "<b>Table 3: Estimated odds ratios (ORs) of participants aged 18 - 64 from the 2015-2016 Canadian community  Health Survey dealing with high depression by work hours, work, health and lifestyle characteristics</b>",format = "html") %>%
  kable_classic(full_width = FALSE,position = "center") %>%
  add_header_above(c(" "=1,"(and 95% CI)"=1,"(and 95% CI)"=1)) %>%
  add_header_above(c(" "=1,"Crude OR"=1,"Adjusted OR"=1))%>%
  pack_rows("Job skill", 2, 4) %>%
  pack_rows("Job security", 5, 7) %>%
  pack_rows("Work authority", 8, 10) %>%
  pack_rows("Co-worker support", 11, 13) %>%
  pack_rows("Average Household income", 14, 18) %>%
  pack_rows("Age", 19, 21) %>%
  pack_rows("Racial background", 22, 23) %>%
  pack_rows("Smoking status", 24, 26) %>%
  pack_rows("BMI", 27, 30) %>%
  pack_rows("Life stress", 31, 33) %>%
  footnote(general = "OR = Odds Ratio CI= confidence interval",
           number = c("Number of individuals (n) used in study = 3433"),
           general_title = "General:",
           title_format = c("italic","underline")
  )


graphdata <-depdata18to64.test
...
ggsummarystats(
  graphdata, x="agebracket", y = "Workhours",
  ggfunc = ggboxplot, xlab = "Age Ranges", ylab ="Average Working hours (weekly)",
  color = "agebracket", palette = c("#00AFBB", "#E7B800", "#FC4E07")
)
