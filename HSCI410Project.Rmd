...
depdata <- select(x, ADM_RNO, DODEP, DOWST, DEPDVSEV,GEN_015,GEN_020, GEN_025,LBFDGHPW,LBFDVPFT,LBFDGOCG,CCC_195,CCC_200,GEO_PRV,DHH_SEX,DHHGMS,DHHGAGE,SDCDGCGT,GENDVHDI,GENDVMHI,INCDGPER,INCDGHH,SWL_005,SWL_015,ALCDVTTM,ALC_015,SMK_005,SMKDVSTY,HWTDGISW,WST_035,SLP_015,EHG2DVR3,SACDVTER,SUI_010)
...
scale21<- function(x){
  case_when(x %in% c("NOC Code first digit: 1, 3, 4, 5, 6")~0,
            x %in% c("NOC Code first digit: 2")~2,
            x %in% c("NOC Code first digit: 6")~3,
            x %in% c("NOC Code first digit: 7")~4,
            x %in% c("NOC Code first digit: 8, 9")~5,
            x %in% c("Valid skip")~9,
            x %in% c("Not stated")~9)
}
...
scale24<- function(x){
  case_when(x %in% c("Lifetime abstainer (never smoked a whole cigarette)")~0,
            ...
            x %in% c("Former occasional smoker (non-smoker now)")~1,
            x %in% c("Former daily smoker (non-smoker now)")~1,
            x %in% c("Current occasional smoker")~9,
            x %in% c("Current daily smoker")~9,
            x %in% c("Not stated")~9)
}
...
scale26<- function(x){
  case_when(x %in% c("Never")~0,
            x %in% c("Rarely")~1,
            x %in% c("Sometimes")~9,
            x %in% c("Most of the time")~9,
            x %in% c("Sometimes")~9,
            x %in% c("All of the time")~9,
            x %in% c("Valid skip")~9,
            x %in% c("Not stated")~9)
}
...
#Renaming and working using this data set. This data set has individuals aged from 18-64. It has 39956 participants. Work satisfaction and job security not filtered out in this data set as >45000 individuals would have been dropped. A separate analysis will be done after filtering out those variables.
...
#Which age grouping to use? 18-25 25-40 40-65
#depdata18to64$DHHGAGE <- sapply(depdata18to64$DHHGAGE,scale7)
...
#Filter out responses that are not stated #42151
...
depdata1 <- depdata.hours %>%
  filter(!LBFDGOCG %in% c("Not stated"),
         ...
         !SDCDGCGT %in% c("Not stated"),
         !DEPDVSEV %in% c("Not stated"),
         !GEN_015 %in% c("Not stated", "Don't know"),
         !GEN_020 %in% c("Not stated", "Don't know"),
         !GEN_025 %in% c("Not stated", "Don't know"),
